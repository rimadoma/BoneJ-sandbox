 ...
 * 2) If there are rows with the given label, but there is not a column with the given heading, then add a column,
 *    and set its value on the first row with the label.
 * 3) If there are rows with the given label, and there's a column with the given heading, then find the first row
 *    which has no value in the column (Double.NaN), and add the new value there. If there are no such rows,
 *    then add a new row.
 *
...
public class ResultsInserter
{
    ...
    /**
     * Adds new data to the underlying ResultsTable according to the policy described in @see ResultsInserter
     *
