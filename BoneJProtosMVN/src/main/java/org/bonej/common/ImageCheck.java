...
public class ImageCheck {
	 ...
	 */
	public static boolean isGrayscale(@Nullable ImagePlus image) {
		if (image == null) {
			return false;
		}
		...
		return (imageType == ImagePlus.GRAY8 || imageType == ImagePlus.GRAY16 || imageType == ImagePlus.GRAY32);
	}
	 ...
	 *            image to test
	 * @param tolerance
	 *            tolerated fractional deviation from equal length [0.0, 1.0]
	 ...
	 *             if 3D environment is lacking
	 * @throws RuntimeException
	 *             if running an incompatible version of ImageJ
	 ...
	 *
	 * @throws ClassNotFoundException
	 *             if either Jave 3D libraries or ImageJ 3D Viewer are not found
	 */
	public static void check3DEnvironment() throws ClassNotFoundException {
		...
		try {
			...
			throw new ClassNotFoundException(
					"ImageJ 3D Viewer is not installed.\n" + "Please install and run the ImageJ 3D Viewer.");
		}
	}
	 ...
	 *
	 * @throws RuntimeException
	 *             if the current IJ version is too old or blacklisted
	 */
	public static void checkIJVersion() {
		String ijVersion = IJ.getVersion();
		if (blacklistedIJVersions.contains(ijVersion)) {
			throw new RuntimeException("The version of ImageJ you are using (v" + IJ.getVersion()
		...
		if (requiredIJVersion.compareToIgnoreCase(ijVersion) > 0) {
			throw new RuntimeException("You are using an old version of ImageJ, v" + IJ.getVersion() + ".\n"
